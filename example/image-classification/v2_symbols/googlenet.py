...
def ConvFactory(data, num_filter, kernel, stride=(1,1), pad=(0, 0), name=None, suffix=''):
    conv = mx.symbol.Convolution(data=data, num_filter=num_filter, kernel=kernel, stride=stride, pad=pad, name='conv_%s%s' %(name, suffix), sleep_time=int(float(forward.pop(0)[1])*1000), backward_sleep_time=int(float(backward.pop(0)[2])*1000))
...
def InceptionFactory(data, num_1x1, num_3x3red, num_3x3, num_d5x5red, num_d5x5, pool, proj, name):
    ...
    # pool + proj
    pooling = mx.symbol.Pooling(data=data, kernel=(3, 3), stride=(1, 1), pad=(1, 1), pool_type=pool, name=('%s_pool_%s_pool' % (pool, name)), sleep_time=int(float(forward.pop(0)[1])*1000), backward_sleep_time=int(float(backward.pop(0)[2])*1000))
...
def get_symbol(num_classes = 1000, **kwargs):
    ...
    pool1 = mx.sym.Pooling(conv1, kernel=(3, 3), stride=(2, 2), pool_type="max", sleep_time=int(float(forward.pop(0)[1])*1000), backward_sleep_time=int(float(backward.pop(0)[2])*1000))
    ...
    pool3 = mx.sym.Pooling(conv3, kernel=(3, 3), stride=(2, 2), pool_type="max", sleep_time=int(float(forward.pop(0)[1])*1000), backward_sleep_time=int(float(backward.pop(0)[2])*1000))
    ...
    pool4 = mx.sym.Pooling(in3b, kernel=(3, 3), stride=(2, 2), pool_type="max", sleep_time=int(float(forward.pop(0)[1])*1000), backward_sleep_time=int(float(backward.pop(0)[2])*1000))
    ...
    pool5 = mx.sym.Pooling(in4e, kernel=(3, 3), stride=(2, 2), pool_type="max", sleep_time=int(float(forward.pop(0)[1])*1000), backward_sleep_time=int(float(backward.pop(0)[2])*1000))
    ...
    pool6 = mx.sym.Pooling(in5b, kernel=(7, 7), stride=(1,1), global_pool=True, pool_type="avg", sleep_time=int(float(forward.pop(0)[1])*1000), backward_sleep_time=int(float(backward.pop(0)[2])*1000))
    flatten = mx.sym.Flatten(data=pool6)
    fc1 = mx.sym.FullyConnected(data=flatten, num_hidden=num_classes, sleep_time=int(float(forward.pop(0)[1])*1000), backward_sleep_time=int(float(backward.pop(0)[2])*1000))
