...
def mobilenet_unit(data, num_filter=1, kernel=(1, 1), stride=(1, 1), pad=(0, 0), num_group=1, if_act=True, prefix=''):
    conv = mx.sym.Convolution(
        ...
        pad=pad,
        no_bias=True,
        name='%s-conv2d'%prefix, virtual_compute=True, sleep_time=int(float(forward.pop(0)[1])*1000), backward_sleep_time=int(float(backward.pop(0)[2])*1000))
    bn = mx.sym.BatchNorm(data=conv, name='%s-batchnorm'%prefix, fix_gamma=False, use_global_stats=False, eps=1e-5, virtual_compute=True)
    if if_act:
        act = relu6(bn, prefix)
        return act
    else:
...
class MobileNetV2(object):
    ...
    def build_network(self, class_num=1000, **configs):
        ...
        pool = mx.sym.Pooling(data=last_fm, kernel=(pool_size, pool_size), stride=(1, 1), 
                              pool_type="avg", name="global_pool", global_pool=True,  virtual_compute=True, sleep_time=int(float(forward.pop(0)[1])*1000), backward_sleep_time=int(float(backward.pop(0)[2])*1000))
        flatten = mx.sym.Flatten(data=pool, name="flatten")
        fc = mx.symbol.FullyConnected(data=flatten, num_hidden=class_num, name='fc',  virtual_compute=True, sleep_time=int(float(forward.pop(0)[1])*1000), backward_sleep_time=int(float(backward.pop(0)[2])*1000))
